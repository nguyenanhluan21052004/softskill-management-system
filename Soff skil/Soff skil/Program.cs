        ...
        options.Events = new JwtBearerEvents
        {
            OnMessageReceived = context =>
            {
                ...
                if (context.Request.Query.TryGetValue("access_token", out var queryToken)
                    && !StringValues.IsNullOrEmpty(queryToken))
                {
                ...
                if (context.Request.Cookies.TryGetValue("access_token", out var cookieToken)
                    || context.Request.Cookies.TryGetValue("token", out cookieToken))
                {
...
{
    c.SwaggerDoc("v1", new OpenApiInfo { Title = "Soff skil", Version = "v1" });
...
using (var scope = app.Services.CreateScope())
{
    ...
    var logger = scope.ServiceProvider.GetRequiredService<ILoggerFactory>().CreateLogger("LegacySchemaPatcher");

    await LegacySchemaPatcher.PatchAsync(dbContext, logger);
}
